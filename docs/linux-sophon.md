...
Use `tools/data/download_kws_model.sh` to retrieve the KWS assets from ModelScope.
...
The canonical Linux wake-word file is `assets/configs/kws/keywords_xiaohui.txt` and
contains only “小慧”. The board pipeline has been verified with a separately
synthesized 16 kHz “小慧，小慧” wake sample.

## Speaker recognition

`SpeakerVerifier` (`sdk/include/ai_audio_assistant/speaker_verifier.hpp`)
...
Reference assets: `tools/data/download_speaker_model.sh` pulls the pre-exported
...
`sherpa-onnx` “speaker-recongition-models” release; `tools/run/run_speaker_demo.sh`
...
`tools/eval/analyze_batch_regression.py` to summarize its JSONL output.
...
not synthesized. The retained `assets/configs/regression/dispatch_speech_stable_v95d_59.txt`
