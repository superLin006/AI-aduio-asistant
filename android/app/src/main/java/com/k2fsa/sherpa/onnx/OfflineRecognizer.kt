...
fun getOfflineModelConfig(type: Int): OfflineModelConfig? {
    when (type) {
        ...
        10 -> {
            val modelDir = "sherpa-onnx-nemo-fast-conformer-ctc-es-1424-int8"
            return OfflineModelConfig(
                nemo = OfflineNemoEncDecCtcModelConfig(
                    model = "$modelDir/model.int8.onnx",
                ),
        ...
        14 -> {
            val modelDir = "sherpa-onnx-paraformer-zh-small-2025-07-16"
            return OfflineModelConfig(
                paraformer = OfflineParaformerModelConfig(
                    model = "$modelDir/model.onnx",
                ),
        ...
        29 -> {
            val modelDir = "sherpa-onnx-zipformer-ru-2025-04-20"
            return OfflineModelConfig(
                transducer = OfflineTransducerModelConfig(
        ...
        30 -> {
            val modelDir = "sherpa-onnx-nemo-parakeet-tdt-0.6b-v3"
            return OfflineModelConfig(
                transducer = OfflineTransducerModelConfig(
                    encoder = "$modelDir/encoder.onnx",
                    decoder = "$modelDir/decoder.onnx",
        ...
        39 -> {
            val modelDir = "sherpa-onnx-zipformer-ctc-small-zh-fp16-2025-07-16"
            return OfflineModelConfig(
                zipformerCtc = OfflineZipformerCtcModelConfig(
        ...
        102 -> {
            val modelDir = "sherpa-onnx-rk3576-10-seconds-paraformer-zh-2023-03-28"
            return OfflineModelConfig(
                paraformer = OfflineParaformerModelConfig(
                    // 关键:三个文件用逗号连接成一个字符串!
                    model = "$modelDir/encoder.rknn,$modelDir/predictor.rknn,$modelDir/decoder.rknn"
                ),
                tokens = "$modelDir/tokens.txt",
                modelType = "paraformer",
                provider = "rknn",      // 指定使用RKNN加速
                numThreads = 1,         // RKNN只支持单线程
            )
}
