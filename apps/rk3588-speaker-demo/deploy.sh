#!/bin/sh
# 一键部署 RK3588 声纹 demo 到板卡并运行（离线 + 在线两个 demo）。
#
# Usage: BOARD_PASS=... sh deploy.sh
#   BOARD_PASS 见 0_Note/board_credentials.md；板卡 scp 子系统不可用，文件走 tar 管道。
#
# 可覆盖的环境变量:
#   BOARD_IP / BOARD_PORT / BOARD_USER / REMOTE_DIR / MODEL / MIC_SECONDS / MIC_DEV
set -eu
...
MODEL=${MODEL:-$DIR/models/eres2netv2_T300_fp.rknn}
OUT=$DIR/build
...
cat "$DIR/online_demo.sh" | $SSH "cat > $REMOTE_DIR/online_demo.sh"

echo "== 板端 demo 1/2：离线（注册 → 识别 → 陌生人拒绝）=="
$SSH "cd $REMOTE_DIR && sh demo.sh" | tee "$OUT/board_run.log"

echo "== 板端 demo 2/2：在线（VAD 流式对话 + 麦克风实时）=="
$SSH "cd $REMOTE_DIR && MIC_SECONDS=${MIC_SECONDS:-8} sh online_demo.sh" | tee "$OUT/board_online_run.log"
