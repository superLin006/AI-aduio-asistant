...
#   仓库布局：sdk/ = 公共库与 CLI，apps/ = 本 demo，delivery/ = 交付产物输出
...
docker run --rm --user "$(id -u):$(id -g)" -e HOME=/tmp \
  ...
  -w /repo/apps/rk3588-speaker-demo \
  sophon-cross-build:latest sh -c '
    set -eu
    mkdir -p build/bin
    COMMON="-std=c++17 -O2 -Wall -fPIC -I/repo/sdk/include -I/sherpa-repo/build-rknn/install/include -I/llm-sdk/3rdparty/common/nlohmann"
    ...
    aarch64-linux-gnu-g++ $COMMON /repo/sdk/src/speaker_verifier.cpp /repo/sdk/cli/speaker_demo.cpp \
      -o build/bin/ai_audio_speaker_demo $LIBS
    aarch64-linux-gnu-g++ $COMMON src/online_speaker_demo.cpp /repo/sdk/src/speaker_verifier.cpp \
