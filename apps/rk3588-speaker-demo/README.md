...
独立的一键演示目录（与 `apps/android-kws-demo/` 同一种组织方式），包含两个 demo：
...
离线 demo 复用应用代码（不复制源码）：`../../sdk/cli/speaker_demo.cpp` + `../../sdk/src/speaker_verifier.cpp`。
...
```sh
cd /home/xh/itc_project/superlin/AI-aduio-asistant/apps/rk3588-speaker-demo
...
```text
delivery/rk3588-speaker-verification/
delivery/rk3588-speaker-verification.tar.gz（由 `tools/build/package_speaker_delivery.sh` 生成）
```
