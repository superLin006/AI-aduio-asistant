...
VAD=${VAD:-./models/silero_vad.onnx}
VAD_PROVIDER=${VAD_PROVIDER:-cpu}
...
echo "=================================================="
$BIN --speaker-model "$MODEL" --speaker-provider rknn \
  --vad "$VAD" --vad-provider "$VAD_PROVIDER" --vad-threshold "$VAD_THRESHOLD" \
  --threshold 0.5 $REG --wav "$W/conversation.wav"
...
echo
echo "在线声纹 demo 结束（声纹在 NPU; VAD=${VAD_PROVIDER}）"
